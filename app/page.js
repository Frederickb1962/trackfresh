                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2 ml-2">
                                  <div className="flex flex-col gap-2" style={{alignItems:"center"}}>
                                    ...
                                    <button onClick={() => { if (it.openDate) { setTrackedItems(prev => prev.map(x => x.id === it.id ? {...x, openDate: "", useByDate: ""} : x)); } else { const today = new Date().toISOString().split("T")[0]; const days = it.daysAfterOpening || 5; const useBy = new Date(Date.now() + days * 86400000).toISOString().split("T")[0]; setTrackedItems(prev => prev.map(x => x.id === it.id ? {...x, openDate: today, useByDate: useBy} : x)); } }} className="rounded-lg px-3 py-1 text-xs font-bold text-white shadow-md" style={{background:"#22c55e",border:`3px solid ${it.daysLeft === null || it.daysLeft > 7 ? "transparent" : it.daysLeft < 0 ? "#ef4444" : it.daysLeft <= 3 ? "#f97316" : "#eab308"}`,textShadow:"0 1px 2px rgba(0,0,0,0.4)"}}>{it.openDate ? "📂 Open" : "📂 Opened"}</button>
