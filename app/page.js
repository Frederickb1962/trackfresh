  ...
  useEffect(() => {
    ...
    async function captureAndScan() {
      ...
      try {
        ...
        const items = data.items || [];
        if (items.length === 0) {
          onError(data.error || "Could not read label. Try again.");
        } else if (items.length === 1 && (!mode || mode === "single")) {
          onResult({ ...items[0] });
        } else {
          if (onMultiResult) onMultiResult(items.slice(0, 6));
          else items.slice(0, 6).forEach(item => onResult({ ...item }));
        }
    ...
    async function start() {
      try {
        ...
        setStatus("scanning");
        // Show button after 1s, auto-trigger AI scan after 3s
        photoTimerRef.current = setTimeout(() => setShowPhotoBtn(true), 1000);
        autoScanTimerRef.current = setTimeout(() => { if (!detectedRef.current) captureAndScan(); }, 3000);
      ...
      {scanError ? <p className="text-sm text-red-600 p-4">{scanError}</p> : (
        <div className="relative overflow-hidden rounded-xl bg-black">
          ...
          <p className="absolute bottom-0 left-0 right-0 text-center text-xs text-white py-2 font-bold" style={{background:"rgba(0,0,0,0.6)"}}>
            ...
            {status === "scanning" && "Point camera at food item..."}
      ...
      <div className="mkt-section mkt-animate mkt-animate-d2">
        ...
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:"0.5rem"}}>
          {[
            ...
            {logo:"https://www.google.com/s2/favicons?domain=kroger.com&sz=128",          name:"Kroger",        href:"https://www.kroger.com"},
            {logo:"https://www.google.com/s2/favicons?domain=walmart.com&sz=128",         name:"Walmart",       href:"https://www.walmart.com/grocery"},
            {logo:"https://www.google.com/s2/favicons?domain=wholefoodsmarket.com&sz=128",name:"Whole Foods",   href:"https://www.wholefoodsmarket.com"},
            {logo:"https://www.google.com/s2/favicons?domain=target.com&sz=128",          name:"Target",        href:"https://www.target.com/c/grocery/-/N-5xt1a"},
            {logo:"https://www.google.com/s2/favicons?domain=costco.com&sz=128",          name:"Costco",        href:"https://www.costco.com/grocery-household.html"},
            {logo:"https://www.google.com/s2/favicons?domain=traderjoes.com&sz=128",      name:"Trader Joe's",  href:"https://www.traderjoes.com"},
            {logo:"https://www.google.com/s2/favicons?domain=instacart.com&sz=128",       name:"Instacart",     href:"https://www.instacart.com"},
          ].map(({logo,name,href}) => (
            <a key={name} href={href} target="_blank" rel="noopener noreferrer" className="mkt-store-card" style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"0.25rem",background:"rgba(255,255,255,0.1)",borderRadius:"14px",padding:"0.5rem 0.65rem",minWidth:"80px",border:"1px solid rgba(255,255,255,0.18)",textDecoration:"none",cursor:"pointer"}}>
...
export default function TrackFreshDashboard() {
  ...
  const [showUseSoonPanel, setShowUseSoonPanel] = useState(false);
  ...
  const handleSmartError = (msg) => { setSmartError(msg); setSmartResult(null); setSmartScanning(false); };

  const takeSmartPhoto = async () => {
    if (smartScanning) return;
    setSmartScanning(true);
    try {
      let base64 = null;
      const video = document.querySelector("#smartScannerVideo");
      if (!video || !video.srcObject) { handleSmartError("Camera not ready. Try again."); return; }
      try {
        const track = video.srcObject.getVideoTracks()[0];
        if (typeof ImageCapture !== "undefined") {
          const ic = new ImageCapture(track);
          const blob = await ic.takePhoto();
          base64 = await new Promise(resolve => { const r = new FileReader(); r.onload = () => resolve(r.result.split(",")[1]); r.readAsDataURL(blob); });
        } else {
      ...
      } catch(e) { handleSmartError("Capture failed: " + e.message); return; }
      const res = await fetch("/api/scan-smart", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ imageData: base64, mediaType: "image/jpeg" }) });
      const data = await res.json();
      const items = data.items || [];
      if (items.length === 0) {
        handleSmartError(data.error || "No items detected. Try again.");
      } else if (items.length === 1 && scanMode === "single") {
        handleSmartResultMulti({ ...items[0] });
      } else {
        setSmartMultiItems(items.slice(0, 6));
        setSelectedSmartItems(items.slice(0, 6).map((_, i) => i));
        setSmartError("");
      }
  ...
  const parseSpokenDate = (transcript) => {
    const months = { january:1, february:2, march:3, april:4, may:5, june:6, july:7, august:8, september:9, october:10, november:11, december:12, jan:1, feb:2, mar:3, apr:4, jun:6, jul:7, aug:8, sep:9, oct:10, nov:11, dec:12 };
    // Normalize spoken year words → digits before matching
    let t = transcript.toLowerCase().trim()
      .replace(/\btwenty[\s-]?twenty[\s-]?six\b/g,   "2026")
      .replace(/\btwenty[\s-]?twenty[\s-]?seven\b/g, "2027")
      .replace(/\btwenty[\s-]?twenty[\s-]?eight\b/g, "2028")
      .replace(/\btwenty[\s-]?twenty[\s-]?nine\b/g,  "2029")
      .replace(/\btwenty[\s-]?twenty[\s-]?five\b/g,  "2025")
      .replace(/\btwenty[\s-]?twenty\b/g,             "2020")
      .replace(/\btwenty[\s-]six\b/g,   "2026")
      .replace(/\btwenty[\s-]seven\b/g, "2027")
      .replace(/\btwenty[\s-]eight\b/g, "2028")
      .replace(/\btwenty[\s-]nine\b/g,  "2029")
      .replace(/\btwenty[\s-]five\b/g,  "2025")
      .replace(/\btwenty[\s-]four\b/g,  "2024");
    // Pattern 1: "month day [year]" — year is optional
  ...
  const handleSpeakToAdd = () => {
    if (!("speechSynthesis" in window)) return;
    const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SR) return;

    // Local flow state — avoids stale closure on React state
    const f = { name: "", category: "Other", location: "Fridge", date: "", cancelled: false };
    let activeRecog = null;

    const stopAll = (closeModal = true) => {
      f.cancelled = true;
      if (activeRecog) { try { activeRecog.abort(); } catch(e) {} activeRecog = null; }
      window.speechSynthesis.cancel();
      if (closeModal) setShowQuickAdd(false);
    };
    ...
    const speak = (text, onDone) => {
      ...
      let fired = false;
      const fire = () => { if (!fired && !f.cancelled) { fired = true; onDone(); } };
      u.onend = fire;
      setTimeout(fire, Math.max(1400, text.split(" ").length / 2.5 * 1000 + 700));
      window.speechSynthesis.speak(u);
    };

    const listen = (onResult, onFail) => {
      if (f.cancelled) return;
      const recog = new SR();
      activeRecog = recog;
      recog.lang = "en-US";
      ...
      recog.maxAlternatives = 3;
      let settled = false;
      const settle = (fn) => { if (settled || f.cancelled) return; settled = true; activeRecog = null; if (!f.cancelled) fn(); };
      const timer = setTimeout(() => settle(onFail), 6000);
      recog.onresult = (e) => {
        clearTimeout(timer);
        const t = e.results[0][0].transcript.trim();
        settle(() => {
          if (/\b(cancel|stop|never mind|nevermind)\b/i.test(t)) { stopAll(); return; }
          onResult(t);
        });
      };
      recog.onerror = () => { clearTimeout(timer); settle(onFail); };
      recog.onend = () => clearTimeout(timer);
      try { recog.start(); } catch(e) { settle(onFail); }
    };

    const parseCat = (t) => {
      if (/produce|fruit|vegeta|fresh/i.test(t)) return "Produce";
      if (/dairy|milk|cheese|yogurt|butter/i.test(t)) return "Dairy";
      if (/meat|chicken|beef|pork|fish|seafood|protein|poultry/i.test(t)) return "Meat";
      if (/pantry|dry|can|grain|pasta|rice|staple/i.test(t)) return "Pantry";
      return "Other";
    };

    const parseLoc = (t) => {
      if (/freez/i.test(t)) return "Freezer";
      if (/pantry/i.test(t)) return "Pantry";
      return "Fridge";
    };
    ...
    const stepItem = () => {
      speak("What's the food item?", () =>
        speak("Please let me know when ready.", () =>
          listen(
            (t) => { f.name = tc(t); setQuickAddName(f.name); speak("Got it, " + f.name + ".", () => stepCategory()); },
            () => speak("I didn't catch that.", () => stepItem())
          )
        )
      );
    };
    ...
    const stepCategory = () => {
      speak("Category: Produce, Dairy, Meat, Pantry, or Other?", () =>
        speak("Please let me know when ready.", () =>
          listen(
            (t) => { f.category = parseCat(t); setQuickAddCategory(f.category); speak("Got it, " + f.category + ".", () => stepLocation()); },
            () => speak("I didn't catch that.", () => stepCategory())
          )
        )
      );
    };
    ...
    const stepLocation = () => {
      speak("Location: Fridge, Freezer, or Pantry?", () =>
        speak("Please let me know when ready.", () =>
          listen(
            (t) => { f.location = parseLoc(t); setQuickAddLocation(f.location); speak("Got it, " + f.location + ".", () => stepDate()); },
            () => speak("I didn't catch that.", () => stepLocation())
          )
        )
      );
    };
    ...
    const stepDate = () => {
      speak("Expiration date?", () =>
        speak("Please let me know when ready.", () =>
          listen(
            (t) => {
              const parsed = parseSpokenDate(t);
              if (parsed) {
                f.date = parsed; setQuickAddDate(parsed);
                speak("Got it.", () => stepConfirm());
              } else {
                speak("I didn't catch a date. Skipping.", () => stepConfirm());
              }
            },
            () => speak("Skipping date.", () => stepConfirm())
          )
        )
      );
    };
    ...
    const stepConfirm = () => {
      speak("Ready to add " + f.name + "?", () =>
        listen(
          (t) => {
            if (/\b(yes|yeah|yep|sure|ok|okay|add|go ahead|do it)\b/i.test(t)) {
              doAdd();
            } else {
              speak("Got it, cancelling.", () => stopAll());
            }
          },
          () => speak("Say yes to add, or cancel to stop.", () => stepConfirm())
        )
      );
    };
    ...
    const doAdd = async () => {
      ...
      let foodInfo = {};
      try {
        const res = await fetch("/api/food-info", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: f.name }) });
      ...
      <div key={activeTab} className="mx-auto max-w-2xl space-y-4 px-4 pt-4 pb-8 tab-enter">
        ...
        {showReceiptScanner && (
          <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 p-4 overflow-y-auto">
            <div className="w-full max-w-lg rounded-xl bg-white px-6 pt-6 shadow-lg my-auto max-h-[calc(100dvh-2rem)] overflow-y-auto" style={{paddingBottom:"max(1.5rem, env(safe-area-inset-bottom))"}}>
          ...
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:"1rem"}}>
            <div style={{background:"linear-gradient(160deg,#064e3b 0%,#065f46 45%,#047857 100%)",borderRadius:"20px",width:"100%",maxWidth:"440px",maxHeight:"90vh",overflow:"auto",padding:"1.25rem"}}>
              ...
              {!smartResult && !smartError && smartMultiItems.length === 0 && (<div>
                ...
                <SmartScanner key={smartScanKey} onResult={handleSmartResultMulti} onError={handleSmartError} captureRef={smartCaptureRef} mode={scanMode} onMultiResult={(items) => { setSmartMultiItems(items); setSelectedSmartItems(items.map((_,i)=>i)); }} />
                {smartScanning ? (
                  <div className="w-full rounded-xl py-3 text-sm font-bold mt-3 btn-green-3d text-center">⏳ Analyzing photo...</div>
                ) : (
                  <button onClick={takeSmartPhoto} className="w-full rounded-xl py-3 text-sm font-bold mt-3 btn-green-3d">
                    {scanMode === "multi" ? "📸 Take Photo — Detect Multiple Items" : "📸 Take Photo"}
                  </button>
                )}
              </div>)}
                    ...
                    {smartMultiItems.map((item, i) => (
                      <div key={i} onClick={() => setSelectedSmartItems(prev => prev.includes(i) ? prev.filter(x => x !== i) : [...prev, i])} style={{border: selectedSmartItems.includes(i) ? "2px solid #16a34a" : "2px solid #e5e7eb", borderRadius:"10px", padding:"0.6rem 0.65rem", cursor:"pointer", background: selectedSmartItems.includes(i) ? "#f0fdf4" : "white", transition:"all 0.12s"}}>
                        <div style={{display:"flex",alignItems:"flex-start",gap:"0.4rem"}}>
                          <input type="checkbox" readOnly checked={selectedSmartItems.includes(i)} style={{marginTop:"2px",accentColor:"#16a34a",flexShrink:0}} />
                          <div style={{minWidth:0}}>
                            <p style={{fontWeight:700,fontSize:"0.8rem",color:"#111",lineHeight:1.25,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{item.name}</p>
                            <p style={{fontSize:"0.65rem",color:"#6b7280",marginTop:"0.15rem"}}>{item.location || "Fridge"} · {item.category || "Other"}</p>
                            {item.dateFound && item.date && <p style={{fontSize:"0.62rem",color:"#16a34a",marginTop:"0.1rem"}}>Exp {item.date}</p>}
                          </div>
                        </div>
                      </div>
                    ))}
...
<button onClick={() => { setBarcodeError(""); setBarcodeDetected(""); setBarcodeItem(null); setBarcodeScanning(false); setShowBarcodeScanner(false); setScanMode(null); setTimeout(() => setShowBarcodeScanner(true), 1000); }} className="mt-2 text-xs text-green-700 underline">{t("tryAgain")}</button>
        ...
        {activeTab === "stores-page" && (
          <div className="space-y-4">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  {href:"https://www.amazon.com/alm/storefront?almBrandId=QW1hem9uIEZyZXNo", domain:"amazon.com",          name:"Amazon Fresh"},
                  {href:"https://www.kroger.com",                                             domain:"kroger.com",           name:"Kroger"},
                  {href:"https://www.walmart.com/cp/food/976759",                             domain:"walmart.com",          name:"Walmart"},
                  {href:"https://www.wholefoodsmarket.com",                                   domain:"wholefoodsmarket.com", name:"Whole Foods"},
                  {href:"https://www.target.com/c/grocery/-/N-5xt1a",                        domain:"target.com",           name:"Target"},
                  {href:"https://www.costco.com/grocery-household.html",                     domain:"costco.com",           name:"Costco"},
                  {href:"https://www.traderjoes.com",                                         domain:"traderjoes.com",       name:"Trader Joe's"},
                  {href:"https://www.instacart.com",                                          domain:"instacart.com",        name:"Instacart"},
                ].map(({href, domain, name}) => (
                  <a key={name} href={href} target="_blank" rel="noopener noreferrer" className="btn-green-3d rounded-2xl px-3 py-4 flex flex-col items-center justify-center gap-2 text-center" style={{textDecoration:"none"}}>
                    <div style={{width:38,height:38,background:"#fff",borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",padding:3,flexShrink:0}}>
                      <img src={`https://www.google.com/s2/favicons?domain=${domain}&sz=64`} alt={name} style={{width:28,height:28,objectFit:"contain"}} onError={e=>{e.target.parentNode.innerHTML='<span style="font-size:1.2rem">🏪</span>';}} />
                    </div>
        ...
        {showUseSoonPanel && (() => {
          const isEs = lang === "es";
          const soonItems = [...itemsWithCountdown]
            .filter(it => it.daysLeft !== null && it.daysLeft <= 7)
            .sort((a, b) => a.daysLeft - b.daysLeft);
          const dayColor = (d) => d <= 0 ? "#f87171" : d <= 2 ? "#fb923c" : d <= 4 ? "#fbbf24" : "#86efac";
          const dayLabel = (d) => {
            if (d < 0) return isEs ? "Vencido" : "Expired";
            if (d === 0) return isEs ? "Hoy" : "Today";
            if (d === 1) return isEs ? "Mañana" : "Tomorrow";
            return isEs ? `${d} días` : `${d} days`;
          };
          return (
            <div style={{position:"fixed",inset:0,zIndex:9999,display:"flex",flexDirection:"column",justifyContent:"flex-end"}} onClick={() => setShowUseSoonPanel(false)}>
              <div onClick={e => e.stopPropagation()} style={{background:"linear-gradient(180deg,#065f46,#064e3b)",borderTop:"1px solid rgba(255,255,255,0.15)",borderTopLeftRadius:"22px",borderTopRightRadius:"22px",padding:"1.25rem 1.25rem 2rem",maxHeight:"80vh",overflowY:"auto",boxShadow:"0 -8px 40px rgba(0,0,0,0.45)"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1rem"}}>
                  <div style={{fontWeight:800,color:"#fff",fontSize:"1.05rem"}}>⚡ {isEs?"Usar Pronto":"Use Soon"}</div>
                  <button onClick={() => setShowUseSoonPanel(false)} style={{background:"rgba(255,255,255,0.12)",border:"none",borderRadius:"999px",width:"2rem",height:"2rem",color:"#fff",fontSize:"1.1rem",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
                </div>
                {soonItems.length === 0 ? (
                  <div style={{textAlign:"center",padding:"2rem 0",color:"#86efac",fontWeight:600,fontSize:"0.9rem"}}>✅ {isEs?"¡Todo está fresco!":"Everything looks fresh!"}</div>
                ) : (
                  <div style={{display:"flex",flexDirection:"column",gap:"0.5rem"}}>
                    {soonItems.map(it => (
                      <div key={it.id} style={{display:"flex",alignItems:"center",gap:"0.75rem",background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.14)",borderRadius:"12px",padding:"0.65rem 0.9rem"}}>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontWeight:700,color:"#fff",fontSize:"0.9rem",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{it.name}</div>
                          <div style={{fontSize:"0.68rem",color:"rgba(255,255,255,0.6)",marginTop:"0.15rem"}}>{it.location ?? "Fridge"} · {it.category ?? ""}</div>
                        </div>
                        <div style={{fontWeight:800,fontSize:"0.78rem",color:dayColor(it.daysLeft),background:`${dayColor(it.daysLeft)}22`,borderRadius:"999px",padding:"0.25rem 0.6rem",whiteSpace:"nowrap",flexShrink:0}}>{dayLabel(it.daysLeft)}</div>
                      </div>
                    ))}
                  </div>
                )}
